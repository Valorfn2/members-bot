...
class Discord_Bot:
  ...
  def __init__(self) -> None:
    ...
    self.allowed_role_ids = ["1119884763830956072", "1182337421564727297","1094655828771995839"]
  ...
  def commands(self):
    ...
    async def on_ready():
        ...
        await self.bot.change_presence(activity=discord.Game(name=".gg/zandx-members"), status=discord.Status.do_not_disturb)
        pass
    ...
    async def tokstockset(ctx, filename):
      ...
      if any(role.id in self.allowed_role_ids for role in user_roles):
        try:
          ...
          #     embed.set_author(name="zandx", icon_url=logo)
    ...
    async def adminpanel(ctx):
      embed = discord.Embed(title='Zandx Commands',
      ...
      embed.add_field(name="!blacklist <serverid> ",
                      value="Blacklist A ID",
                      inline=True)
      embed.add_field(name="!whitelist <serverid> ",
    ...
    @self.bot.command(name="whitelist", description="Whitelist A User.")
    async def whitelist(ctx, userid: int):
      ...
      if any(role.id in self.allowed_role_ids for role in user_roles):
        ...
        embed.add_field(name="**Missing Perms What are you trying lil bro.**",
    ...
    @self.bot.command(name="blacklist", description="Blacklist A User.")
    ...
    async def stock(ctx):
      try:
        ...
        #     embed.set_author(name="zandev", icon_url=logo)
        embed.set_thumbnail(
            url=
            logo
        )
    ...
    async def join(ctx, serverid: int = None):
      ...
      if serverid == self.blistedservids:
        ...
        embed.add_field(name="**Zandx id**",
                        value=f"Zandx Is Blacklisted. Try Your Own Server.",
                        inline=False)
        embed.set_footer(
            text=footer,
            icon_url=
      ...
      if serverid in self.used_server_ids:
        if self.used_server_ids[serverid] != user.id:
          ...
          embed.add_field(
              name="**Error**",
              value=
              f"`{serverid}`: Haha trying to use a alt use your brain and go back on your main you nut.",
              inline=False)
          embed.set_footer(
              text=footer,
              icon_url=
      ...
      embed = discord.Embed(title='**Zandev**', color=0x1B2631)
      embed = discord.Embed(description=f'Adding **{self.aa}** Zandx Members To ``{serverid}`` \n\n ``Queue`` \n **1/20**', title="Zandev")
      #     embed.set_author(name="zandx", icon_url=logo)
      embed.set_footer(
          text=footer,
          icon_url=
    ...
    async def bug(ctx, bug: str):
      ...
      embed.add_field(name="**Bug**",
                      value=f"Bug Reported, ks Will Fix Shortly!",
                      inline=False)
    ...
    async def servers(ctx):
      ...
      if num_servers == 100:
        embed = discord.Embed(title='**We full brody**', color=0x1B2631)
        #     embed.set_author(name="zandx", icon_url=logo)
        embed.set_thumbnail(
            url=
            logo
        ...
        embed.add_field(
            name="**Amount**",
            value=
            f"This Bot Is At The Max Servers Of 100. Ping ks Or Staff To Clear.",
            inline=False)
      ...
      embed.set_footer(
          text=" discord.gg/zandev - ks & andree4real ",
          icon_url=
          logo
      )
      await ctx.send(embed=embed)
